...
class Raccoon extends BotApi {
     ...
     */
    constructor(token, localUrl) {
        super(token, localUrl);
    ...
    registerFeature(owner, feature) {
        if (owner == undefined || feature == undefined) throw new Error(`owner or feature undefined`);
    ...
    getActivity(owner, name) {
        ...
        if (activity === undefined) {
            throw new Error(`Owner '${owner}' doesn't have activity '${name}'.`);
        }
    ...
    async _handleCallbackQuery(context) {
        const { method, params, featureName, owner } = decodeCallbackData(context.data);
    ...
    cleanup(owner, name) {
        ...
        if (Object.keys(this.activityState[owner]).length == 0) delete this.activityState[owner];
        if (this.debug) {
